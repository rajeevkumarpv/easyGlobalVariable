import pickle
import config as global_gv

global_gv.gv_table_globals = {}
...
class GlobalVariableManager:
    ...
    def __getitem__(self, key):
        """Retrieve a variable."""
        if key in global_gv.gv_table_globals:
            return global_gv.gv_table_globals[key]
        raise KeyError(f"Global variable '{key}' not found.")

    def __setitem__(self, key, value):
        """Store a variable."""
        global_gv.gv_table_globals[key] = value

    def __delitem__(self, key):
        """Delete a variable using 'del gv[key]'."""
        if key in global_gv.gv_table_globals:
            del global_gv.gv_table_globals[key]
        else:
            raise KeyError(f"Global variable '{key}' not found.")
    ...
    def get(self, key, default=None):
        """Retrieve a variable with a default value if not found."""
        return global_gv.gv_table_globals.get(key, default)
    ...
    def startswith(self, prefix):
        ...
        subset = {k: v for k, v in global_gv.gv_table_globals.items()
        ...
        try:
            with open(filename, 'rb') as f:
                ...
                if isinstance(data, dict):
                    global_gv.gv_table_globals.update(data)
    ...
    def __repr__(self):
        return f"<GlobalVariableManager storing {len(global_gv.gv_table_globals)} items>"
